...
int main(int argc, char * argv[]){
	int portNo, textLength, keyLength, sockfd, newsockfd, nText, nKey, pID, checkSockSending = 3, checkSockRecieving = 2;
	...
	listen(sockfd,5);										//5 concurent connections max
	...
	while(1){													//infinite loop
		...
		if(newsockfd < 0){										//if can't set up new socket then error
			error("ERROR on accept");
		}
		pID = fork();											//create child
		if(pID < 0){											//if the child is 0 then error
			error("Error forking");
			...
			nText = read(newsockfd, textArray, 2047);			//read the text data from otp_dec
			if(nText < 0){										//if its 0 then no data could be read
				error("Error reading from socket");
			}
			bzero(keyArray, 2047);								//set the buffer size for key array
			nKey = read(newsockfd, keyArray, 2047);				//read the key data from otp_dec
			...
			for(int x = 0; x < strlen(textArray)-1; x++){		//for loop for the length of the array
				...
				cipherNumber = textNumber - keyNumber;		//find the new value for the cipher
				if(cipherNumber < 0){						//if less than 0 then add 27 to get number
			...
			nText = write(newsockfd, cipherArray, 2047);	//return the decrypted text to otp_dec
